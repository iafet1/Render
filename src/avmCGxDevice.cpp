...
constexpr char FILE_NAME[] = "avmCGxDevice.cpp";
...
namespace avm::graphics {
    ...
    // глобальный указатель на графическое устройство
    static CGxDevice* g_gxDevice {nullptr};
    ...
    void CGxDevice::destroyInstance()
    {
    ...
    void CGxDevice::initFunctionsInstance(VkInstance instance)
    {
        #if defined(_LOG) || defined(_DEBUG)
            // VK_EXT_debug_utils ( VK_EXT_DEBUG_UTILS_EXTENSION_NAME ) — расширение экземпляра
    ...
    void CGxDevice::initFunctionsDevice()
    {
    }

    const char *CGxDevice::vkResultToString(VkResult result)
    {
        switch (result)
        {
        case VK_NOT_READY:
